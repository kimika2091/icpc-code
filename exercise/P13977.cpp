...
int query(int l, int r, ll c) {
	...
	int cnt = 0;
	if (p == q) {
		for (int i = l; i <= r; i++) cnt += (a[i] + tag[p]) < c;
		return cnt;
	}
	for (int i = l; i <= R[p]; i++) cnt += (a[i] + tag[p]) < c;
	for (int i = L[q]; i <= r; i++) cnt += (a[i] + tag[q]) < c;
	for (int i = p + 1; i <= q - 1; i++) {
		cnt += lower_bound(sorted[i].begin(), sorted[i].end(), c - tag[i]) - sorted[i].begin();
	}
	return cnt;
}
